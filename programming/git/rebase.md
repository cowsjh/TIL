...
[Git Basic](./git_basics.md)의 `git merge`가 "합친 기록을 남기는" 쪽이라면, rebase는 "합친 적 없던 것처럼 이어 붙이는" 쪽이다.
...
[Git Basic](./git_basics.md)의 `git revert`와 비교하면 차이가 분명하다 — revert는 "반대 커밋을 **새로 추가**"해 원래 기록을 남기는 안전한 방식(공유 브랜치 OK), rebase는 "원래 커밋을 **바꿔치기**"하는 방식(내 것만).
